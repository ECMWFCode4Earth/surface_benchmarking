...
class LDAS_config(object):
    ...
    def check_contents(self):
        ...
        #        self.EXPNAME=["ERA5 control", "SEKF SM analysis"]
        self.config_file = None
        self.extract_SM = False  # Extract soil moisture grib files from MARS (required for preprocessing)
        self.extract_ST = False  # Extract soil temperature grib files from MARS (required for preprocessing)
        self.pre_process_SM = (
           True  # Preprocess soil moisture (required for validation)
        )
        self.pre_process_ST = ( True
  # Preprocess soil temperature (required for validation)
        )
        self.validate_SM = True  # Validate soil moisture
        self.validate_ST = True  # Validate soil temperature

        self.validation_times=["all_times"] # 00_UTC and 12_UTC are optional
        self.climate_classes=["all_climate"]
        self.land_classes=["all_land"] # "Grassland" and "Croplands" optional for 'USCRN', 'SNOTEL', 'SCAN', 'SMOSMANIA', 'TERENO' from 2019

        self.Network = ['USCRN', 'SCAN', 'SNOTEL', 'REMEDHUS', 'OZNET','SMOSMANIA','TERENO']
          # Can be any combination of 'USCRN', 'SNOTEL', 'SCAN', 'SMOSMANIA', 'REMEDHUS', 'OZNET', 'TERENO'
        #self.in_situ_dir = "/Users/dadf/landver_test_21_04_23/ISMN_data/in_situ_data/pre_processed_data"  # Grib directory for in situ soil moisture files
        ...
        self.SM_units = "Volumetric"  # Units of evaluated RMSD and bias scores. Can be 'Volumetric' or 'Dimensionless':
        self.ST_units = "K"  # Units of soil temperature (K)
        self.validation_layer = [
            "Surface", "Rootzone"
        ]  # Soil moisture layers - SSM is surface soil moisture (0-7 cm depth), RSM is root-zone soil moisture (0-1 m depth)
        self.ST_quality_control = False  # Screen SM where soil temperature less than specified value ST_QC_threshold
        ...
        Networks = ['USCRN','SCAN','SNOTEL','REMEDHUS','OZNET','SMOSMANIA','TERENO']
